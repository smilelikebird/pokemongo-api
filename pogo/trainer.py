...
class Trainer(object):
    def __init__(self, auth, session):
        ...
        self.logger = Logger('Trainer').get()
    ...
    def findBestPokemon(self):
        ...
        #with open('logs/lastpos.log', 'wr'): pass
        with open('logs/lastpos.log', 'w') as f : 
            f.write(str("%f, %f" % (latitude, longitude)))
    ...
    #before, step was 7.5
    def walkTo(self, olatitude, olongitude, epsilon=10, step=15):
        if step >= epsilon:
    ...
    def getInventory(self):
        ...
        #self.logger.info(self.session.getInventory())
    ...
    def walkAndSpin(self, fort):
        ...
        if fort:
            ...
            # Walk over
            self.walkTo(fort.latitude, fort.longitude, step=5)
            # Give it a spin
        ...
        while True:
            try:
                ...
                if pokemon is None:
                    ...
                    for fort in forts:
                        response = self.walkAndSpin(fort)
            ...
            # Catch problems and reauthenticate
                ...
                with open('logs/lastpos.log') as f:
                    lastpos = f.readlines()

                logging.critical('GeneralPogoException raised: %s', e)
                logging.critical('Last position is : %s', lastpos[0])
                self.session = self.session.reauthenticate()
                time.sleep(cooldown)
                cooldown *= 2
                while self.session is None:
                    logging.critical('Session is empty, retry reauthenticate')
                    time.sleep(cooldown)
                    self.session = self.auth.authenticate(locationLookup=lastpos[0])
